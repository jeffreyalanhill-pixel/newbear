...
const KEYS = {
  ...
  // ---- Customer Care ----
  customerCareTasks: 'cc_tasks',
  customerCareTemplates: 'cc_templates',
  // ---- Custom board cards (appointments workflow board) ----
  customBoardCards: 'custom_board_cards',
  // ---- Custom workflow steps (user-defined Kanban columns) ----
  customWorkflowSteps: 'custom_workflow_steps',
  // ---- Workflow step settings (user overrides for built-in & custom columns) ----
  workflowStepSettings: 'workflow_step_settings',
  // ---- Versioned shop configuration (workflow steps, locations, views, rules) ----
  shopConfig: 'shop_config',
  // ---- Rewards & Membership ----
  rewardsPrograms: 'rw_programs',
  membershipPlans: 'rw_plans',
  customerRewards: 'rw_customers',
  rewardTransactions: 'rw_transactions',
};
...
export const db = {
  ...
  rewardsPrograms() { return db.get(KEYS.rewardsPrograms) || []; },
  membershipPlans() { return db.get(KEYS.membershipPlans) || []; },
  customerRewards() { return db.get(KEYS.customerRewards) || []; },
  rewardTransactions() { return db.get(KEYS.rewardTransactions) || []; },
  customerCareTasks() { return db.get(KEYS.customerCareTasks) || []; },
  customerCareTemplates() { return db.get(KEYS.customerCareTemplates) || []; },
  customBoardCards() { return db.get(KEYS.customBoardCards) || []; },
  customWorkflowSteps() { return db.get(KEYS.customWorkflowSteps) || []; },
  workflowStepSettings() { return db.get(KEYS.workflowStepSettings) || {}; },
  shopConfig() { return db.get(KEYS.shopConfig); }, // null until seeded — caller owns defaults
  ...
  saveRewardsPrograms(a) { return db.set(KEYS.rewardsPrograms, a); },
  saveMembershipPlans(a) { return db.set(KEYS.membershipPlans, a); },
  saveCustomerRewards(a) { return db.set(KEYS.customerRewards, a); },
  saveRewardTransactions(a) { return db.set(KEYS.rewardTransactions, a); },
  saveCustomerCareTasks(a) { return db.set(KEYS.customerCareTasks, a); },
  saveCustomerCareTemplates(a) { return db.set(KEYS.customerCareTemplates, a); },
  saveCustomBoardCards(a) { return db.set(KEYS.customBoardCards, a); },
  saveCustomWorkflowSteps(a) { return db.set(KEYS.customWorkflowSteps, a); },
  saveWorkflowStepSettings(o) { return db.set(KEYS.workflowStepSettings, o); },
  saveShopConfig(o) { return db.set(KEYS.shopConfig, o); },
  ...
  customBoardCardById(id) { return db.customBoardCards().find(c => c.id === id); },
  ...
  customerRewardByCustomerId(customerId) { return db.customerRewards().find(r => r.customerId === customerId); },
  rewardTransactionsForCustomer(customerId) { return db.rewardTransactions().filter(t => t.customerId === customerId); },
  membershipPlanById(id) { return db.membershipPlans().find(p => p.id === id); },
  ...
  segmentMembers(segmentId) {
    ...
    switch (criteria.kind) {
      ...
      case 'unpaid_invoices':
        ...
        break;
      case 'rewards_member':
        customers = customers.filter(c => db.customerRewardByCustomerId(c.id)?.membershipStatus === 'active');
        break;
      case 'rewards_non_member':
        customers = customers.filter(c => !db.customerRewardByCustomerId(c.id));
        break;
      case 'rewards_vip':
        customers = customers.filter(c => db.customerRewardByCustomerId(c.id)?.tier === 'vip');
        break;
      case 'rewards_points_available': {
        const prog = db.rewardsPrograms()[0];
        const minPts = prog?.minimumPointsToRedeem || 500;
        customers = customers.filter(c => (db.customerRewardByCustomerId(c.id)?.pointsBalance || 0) >= minPts);
        break;
      }
      default:
...
function seed() {
  ...
  db.saveSegments([
    ...
    { id: 'seg_no_show', name: 'No-Shows', criteria: { kind: 'no_show' }, computed: true,
      description: 'Had an RO marked as a no-show. Real — set by the Appointments no-show action.' },
    { id: 'seg_unpaid', name: 'Unpaid Invoices', criteria: { kind: 'unpaid_invoices' }, computed: true,
      description: 'Has at least one invoice with a remaining balance. Real — derived from invoice.balance.' },
    { id: 'seg_rw_members', name: 'Rewards Members', criteria: { kind: 'rewards_member' }, computed: true,
      description: 'Enrolled in the Rewards program with active status. Real.' },
    { id: 'seg_rw_vip', name: 'Rewards VIP Tier', criteria: { kind: 'rewards_vip' }, computed: true,
      description: 'Members who have reached VIP tier ($2,500+ lifetime spend). Real.' },
    { id: 'seg_rw_points', name: 'Points Ready to Redeem', criteria: { kind: 'rewards_points_available' }, computed: true,
      description: 'Members with 500+ points available to redeem. Real.' },
    { id: 'seg_rw_no_program', name: 'Not Enrolled in Rewards', criteria: { kind: 'rewards_non_member' }, computed: true,
      description: 'Customers not yet enrolled in any Rewards plan. Real.' },
  ]);
  // ---- Rewards & Membership seed ----
  db.saveRewardsPrograms([{
    id: 'rp_default', name: 'AutoBook Rewards', isActive: true,
    pointsPerDollar: 1, redemptionRate: 0.01, minimumPointsToRedeem: 500,
    createdAt: daysAgoISO(365),
  }]);
  db.saveMembershipPlans([
    { id: 'plan_free',  name: 'Rewards Member',  price: 0,    billingCycle: null,    pointsMultiplier: 1.0, description: 'Free loyalty rewards. Earn 1 pt per $1 spent.' },
    { id: 'plan_oil',   name: 'Oil Change Club',  price: 9.99, billingCycle: 'month', pointsMultiplier: 1.5, description: 'Monthly oil changes + 1.5× points + 5% off parts.' },
    { id: 'plan_vip',   name: 'VIP Care Plan',    price: 29,   billingCycle: 'month', pointsMultiplier: 2.0, description: '2× points + 10% off all services + priority scheduling.' },
    { id: 'plan_fleet', name: 'Fleet Family',     price: 49,   billingCycle: 'month', pointsMultiplier: 1.5, description: 'Multi-vehicle flat-rate pricing + monthly fleet report.' },
  ]);
  db.saveCustomerRewards([
    { id: 'cr_maria',  customerId: 'c_maria',  pointsBalance: 1240, lifetimePoints: 1580, membershipPlanId: 'plan_vip',  membershipStatus: 'active', tier: 'gold',   enrolledAt: daysAgoISO(180), notes: '' },
    { id: 'cr_david',  customerId: 'c_david',  pointsBalance: 680,  lifetimePoints: 950,  membershipPlanId: 'plan_free', membershipStatus: 'active', tier: 'silver', enrolledAt: daysAgoISO(120), notes: '' },
    { id: 'cr_robert', customerId: 'c_robert', pointsBalance: 2100, lifetimePoints: 2800, membershipPlanId: 'plan_vip',  membershipStatus: 'active', tier: 'vip',    enrolledAt: daysAgoISO(300), notes: '' },
    { id: 'cr_tom',    customerId: 'c_tom',    pointsBalance: 440,  lifetimePoints: 600,  membershipPlanId: 'plan_oil',  membershipStatus: 'active', tier: 'bronze', enrolledAt: daysAgoISO(90),  notes: '' },
    { id: 'cr_chrisb', customerId: 'c_chrisb', pointsBalance: 890,  lifetimePoints: 1100, membershipPlanId: 'plan_free', membershipStatus: 'active', tier: 'silver', enrolledAt: daysAgoISO(60),  notes: '' },
    { id: 'cr_james',  customerId: 'c_james',  pointsBalance: 150,  lifetimePoints: 150,  membershipPlanId: 'plan_free', membershipStatus: 'active', tier: 'bronze', enrolledAt: daysAgoISO(25),  notes: '' },
  ]);
  db.saveRewardTransactions([
    { id: 'rt_1', customerId: 'c_maria',  sourceType: 'invoice', sourceId: null, pointsChange:  340, description: 'Invoice payment — 2× VIP bonus',  createdAt: daysAgoISO(5),  createdByEmployeeId: 'e_jeff' },
    { id: 'rt_2', customerId: 'c_maria',  sourceType: 'redemption', sourceId: null, pointsChange: -500, description: 'Redemption applied — $5.00 off', createdAt: daysAgoISO(15), createdByEmployeeId: 'e_sara' },
    { id: 'rt_3', customerId: 'c_robert', sourceType: 'invoice', sourceId: null, pointsChange:  560, description: 'Invoice payment — 2× VIP bonus',  createdAt: daysAgoISO(3),  createdByEmployeeId: 'e_jeff' },
    { id: 'rt_4', customerId: 'c_david',  sourceType: 'invoice', sourceId: null, pointsChange:  220, description: 'Invoice payment',                  createdAt: daysAgoISO(7),  createdByEmployeeId: 'e_jeff' },
    { id: 'rt_5', customerId: 'c_tom',    sourceType: 'invoice', sourceId: null, pointsChange:  180, description: 'Invoice payment — 1.5× Oil Club',  createdAt: daysAgoISO(10), createdByEmployeeId: 'e_jeff' },
    { id: 'rt_6', customerId: 'c_chrisb', sourceType: 'promo',   sourceId: null, pointsChange:  100, description: 'Welcome bonus',                    createdAt: daysAgoISO(60), createdByEmployeeId: 'e_jeff' },
  ]);
