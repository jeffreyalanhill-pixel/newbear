...
function seed() {
  ...
  for (let d = 0; d < 5; d++) {
    techShiftIds.forEach((employeeId) => {
      ...
      shifts.push({
        id: db.nextId('shift'), employeeId, date: dateStr, weekStart: mon.toISOString().slice(0, 10),
        ...
        status: dateStr < today ? 'completed' : 'scheduled', breakMinutes: 30,
      });
    });
  }
  ...
  // Simple default: 3x the reorder point, rounded to a sane order quantity.
