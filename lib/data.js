...
export const db = {
  ...
  // §D — segment membership. Phase 1 supports two criteria kinds: "all"
  // customers, or customers whose any vehicle matches a given make. Always
  // excludes do-not-contact customers, per the spec's audience-preview rule.
  segmentMembers(segmentId) {
    const segment = db.segmentById(segmentId);
    if (!segment) return [];
    let customers = db.customers().filter(c => !c.doNotContact);
    if (segment.criteria?.vehicleMake) {
      const make = segment.criteria.vehicleMake.toLowerCase();
