...
util.sendCampaign = (campaignId) => {
  ...
  const template = db.templateById(campaign.templateId);
  if (!template) throw new Error('Campaign has no template selected.');
  const recipients = db.segmentMembers(campaign.segmentId);

  const communications = db.communications();
  recipients.forEach((customer) => {
    const vars = { ...mergeVarsFor(customer), couponCode: campaign.couponCode || '' };
    communications.push({
      id: db.nextId('comm'),
      customerId: customer.id,
      channel: 'email',
      direction: 'out',
      subject: util.renderTemplate(template.subject, vars),
      body: util.renderTemplate(template.body, vars),
  ...
  campaign.metrics = { ...(campaign.metrics || {}), sent: recipients.length };
