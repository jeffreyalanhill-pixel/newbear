...
util.customerName = (c) => {
  ...
  return c.name || c.company || 'Customer not assigned';
};
util.customerShort = (c) => {
  if (!c) return '';
  if (c.firstName) return `${c.firstName}${c.lastName ? ' ' + c.lastName.charAt(0) + '.' : ''}`;
  if (c.name) {
    const parts = c.name.trim().split(' ');
    return parts.length > 1 ? `${parts[0]} ${parts[1].charAt(0)}.` : parts[0];
  }
  return c.company || 'Customer not assigned';
};
...
const STATUS_META = {
  ...
  request:          { label: 'Request / Estimate', badgeClass: 'badge-blue',   kanbanCol: null },
  estimate:         { label: 'Estimate',           badgeClass: 'badge-blue',   kanbanCol: null },
  walk_in:          { label: 'Walk-in',            badgeClass: 'badge-purple', kanbanCol: null },
  quality_check:    { label: 'Quality Check',      badgeClass: 'badge-purple', kanbanCol: null },
  ready:            { label: 'Ready',              badgeClass: 'badge-green',  kanbanCol: 'ready' },
  waiting_bay:      { label: 'Waiting Bay',        badgeClass: 'badge-purple', kanbanCol: null },
  ready_for_pickup: { label: 'Ready for Pickup',   badgeClass: 'badge-green',  kanbanCol: 'ready' },
  picked_up_closed: { label: 'Picked Up / Closed', badgeClass: 'badge-gray',   kanbanCol: null },
...
util.computeFlags = () => {
  ...
  // Jobs "past estimate" are NOT flagged here — that signal is already
  // visible per-job on the Live Jobs kanban card (a "Late" badge), so
  // repeating it in Flags would just be the same fact twice. "Tomorrow over
  // capacity" moved to util.coachingInsights() — it's a forward-looking
  // planning note, not a same-day factual issue, and Tomorrow Preview's own
  // alert already covers it where it's most relevant.
  ...
  const activeNeedingAssignment = jobs.filter((j) => ['waiting', 'in_progress'].includes(j.status));
  const noTech = activeNeedingAssignment.filter((j) => !j.techId);
  if (noTech.length) {
    flags.push({
      level: 'amber',
      icon: 'part',
      title: `${noTech.length} active job${noTech.length > 1 ? 's' : ''} with no tech assigned`,
      sub: noTech.map((j) => j.ro).join(', '),
      href: '#repair-orders',
      roIds: noTech.map((j) => j.id),
    });
  }
  const noBay = activeNeedingAssignment.filter((j) => !j.bayId);
  if (noBay.length) {
    flags.push({
      level: 'amber',
      icon: 'car',
      title: `${noBay.length} active job${noBay.length > 1 ? 's' : ''} with no bay assigned`,
      sub: noBay.map((j) => j.ro).join(', '),
      href: '#repair-orders',
      roIds: noBay.map((j) => j.id),
    });
  }
...
util.coachingInsights = () => {
  ...
  // Parts-delayed jobs are NOT repeated here — util.computeFlags() already
  // surfaces them factually ("Parts not arrived") with no extra recommendation
  // to add, so a second copy here would just be the same issue twice.

  const tomorrow = util.tomorrowPreview();
  if (tomorrow.overCapacity) {
    insights.push({
      icon: 'calendar',
      title: `Tomorrow is booked ${round2(tomorrow.estHours - tomorrow.capacity)} hrs over capacity`,
      body: `${tomorrow.appts} jobs booked — reschedule one or confirm extra tech coverage.`,
    });
  }

  const aroTarget = db.settings().aroTarget || 0;
  if (aroTarget) {
    const pendingByValue = db.jobs()
      .filter((j) => j.approvalStatus === 'pending')
      .map((j) => ({ job: j, value: (j.recommended || []).reduce((s, r) => s + (r.price || 0), 0) || j.total || 0 }))
      .filter((x) => x.value > aroTarget * 1.5)
      .sort((a, b) => b.value - a.value);
    if (pendingByValue.length) {
      const top = pendingByValue[0];
      insights.push({
        icon: 'part',
        title: `${util.fmtMoney0(top.value)} job waiting on approval`,
        body: `${util.vehicleLabel(db.vehicleById(top.job.vehicleId))} (${top.job.ro}) — worth a personal call to close it.`,
      });
    }
  }

  // One aggregated note, not one per affected tech — keeps this list short
  // enough for the dashboard's "2-3 insights max" right-panel slot.
  const comebackTechIds = new Set(
    db.jobs().filter((j) => j.isComeback && util.isThisWeek(j.createdAt)).map((j) => j.techId)
  );
  const comebackTechs = [...comebackTechIds].map((id) => db.techById(id)).filter(Boolean);
  if (comebackTechs.length) {
    insights.push({
      icon: 'comeback',
      title: `${comebackTechs.length} tech${comebackTechs.length > 1 ? 's' : ''} with a comeback flag this week`,
      body: `${comebackTechs.map((t) => t.firstName).join(', ')} — review for a quality follow-up.`,
    });
  }
  return insights;
};
...
util.convertLead = (leadId) => {
  ...
  workflow.linkEntities('lead', lead.id, 'customer', customer.id, 'crm_lead_to_customer');
  workflow.recordWorkflowEvent('customer', customer.id, 'lead_converted', `${util.customerName(customer)} converted from lead`, { customerId: customer.id, leadId: lead.id });
...
util.submitBooking = ({ serviceIds, preferredDate, preferredSlot, customer, vehicle, couponCode, notes }) => {
  ...
  workflow.recordWorkflowEvent('booking', booking.id, 'booking_requested', 'Booking request submitted', { customerId: c.id, vehicleId: v.id, bookingId: booking.id });
  return booking;
};
...
util.confirmBooking = (bookingId, opts = {}) => {
  ...
  // Data safety (§17): never silently create a second RO off the same
  // booking — confirmBooking is only ever meant to run once per booking.
  if (booking.roId) throw new Error(`Booking already confirmed as ${db.jobById(booking.roId)?.ro || booking.roId}.`);
  ...
  workflow.linkEntities('booking', booking.id, 'job', ro.id, 'booking_to_appointment');
  workflow.recordWorkflowEvent('booking', booking.id, 'booking_confirmed', 'Booking confirmed', { customerId: customer.id, bookingId: booking.id, roId: ro.id });
  workflow.recordWorkflowEvent('job', ro.id, 'appointment_scheduled', `${ro.ro} scheduled`, { customerId: customer.id, vehicleId: vehicle.id, roId: ro.id, bookingId: booking.id });
...
util.declineBooking = (bookingId) => {
  ...
  db.saveBookings(bookings);
  workflow.recordWorkflowEvent('booking', booking.id, 'booking_declined', 'Booking request declined', { customerId: booking.customerId, bookingId: booking.id });
  return booking;
};
...
util.checkIn = (roId) => {
  ...
  const saved = saveJob(ro);
  workflow.recordWorkflowEvent('job', ro.id, 'customer_checked_in', `Customer checked in for ${ro.ro}`, { customerId: ro.customerId, vehicleId: ro.vehicleId, roId: ro.id });
  return saved;
};
...
util.startJob = (roId, bayId, techId) => {
  ...
  const saved = saveJob(ro);
  workflow.recordWorkflowEvent('job', ro.id, 'ro_in_progress', `${ro.ro} started`, { customerId: ro.customerId, roId: ro.id });
  return saved;
};
...
util.holdJob = (roId, reason) => {
  ...
  const saved = saveJob(ro);
  workflow.recordWorkflowEvent('job', ro.id, 'ro_on_hold', `${ro.ro} on hold — ${reason}`, { customerId: ro.customerId, roId: ro.id });
  return saved;
};
...
util.resumeJob = (roId) => {
  ...
  ro.status = 'in_progress';
  const saved = saveJob(ro);
  workflow.recordWorkflowEvent('job', ro.id, 'ro_resumed', `${ro.ro} resumed`, { customerId: ro.customerId, roId: ro.id });
  return saved;
};

util.moveToQualityCheck = (roId) => {
  const ro = db.jobById(roId);
  requireStatus(ro, ['in_progress']);
  ro.status = 'quality_check';
  const saved = saveJob(ro);
  workflow.recordWorkflowEvent('job', ro.id, 'ro_quality_check', `${ro.ro} moved to Quality Check`, { customerId: ro.customerId, roId: ro.id });
  return saved;
};

util.markReady = (roId) => {
  const ro = db.jobById(roId);
  requireStatus(ro, ['in_progress', 'quality_check']); // quality_check is a valid prior state
  ro.status = 'ready';
  ro.completedAt = new Date().toISOString();
  const saved = saveJob(ro);
  workflow.recordWorkflowEvent('job', ro.id, 'ro_ready', `${ro.ro} marked ready`, { customerId: ro.customerId, roId: ro.id });
  return saved;
};
...
util.setDviItem = (roId, item, status, note) => {
  const ro = db.jobById(roId);
  const isFirstDviItem = !(ro.dvi || []).length;
  ro.dvi = ro.dvi || [];
  ...
  const saved = saveJob(ro);
  if (isFirstDviItem) workflow.recordWorkflowEvent('job', ro.id, 'dvi_started', `Inspection started on ${ro.ro}`, { customerId: ro.customerId, roId: ro.id });
  if (status === 'yellow' || status === 'red') {
    workflow.recordWorkflowEvent('job', ro.id, 'dvi_finding', `${status.toUpperCase()} finding on ${ro.ro}: ${item}`, { customerId: ro.customerId, roId: ro.id, metadata: { item, status } });
  }
  return saved;
};
...
util.requestApproval = (roId) => {
  ...
  const saved = saveJob(ro);
  workflow.recordWorkflowEvent('job', ro.id, 'approval_requested', `Approval requested for ${ro.ro}`, { customerId: ro.customerId, roId: ro.id });
  return saved;
};
...
util.resolveApproval = (roId, approved) => {
  ...
  const saved = saveJob(ro);
  workflow.recordWorkflowEvent('job', ro.id, approved ? 'ro_approval_approved' : 'ro_approval_declined', `${ro.ro} recommended work ${approved ? 'approved' : 'declined'}`, { customerId: ro.customerId, roId: ro.id });
  return saved;
};
...
util.createInvoiceFromRO = (roId) => {
  ...
  workflow.linkEntities('job', ro.id, 'invoice', invoice.id, 'ro_to_invoice');
  if (ro.quoteId) workflow.linkEntities('quote', ro.quoteId, 'invoice', invoice.id, 'quote_to_invoice');
  workflow.recordWorkflowEvent('invoice', invoice.id, 'invoice_created', `${invoice.number} created from ${ro.ro}`, { customerId: ro.customerId, roId: ro.id, invoiceId: invoice.id });

  return invoice;
};

// `extra` is additive ({ reference } so far) — existing 3-arg call sites
// (modules/invoices.js's payment modal) keep working unchanged.
util.recordPayment = (invoiceId, amount, method, extra = {}) => {
  const invoices = db.invoices();
  const invoice = invoices.find((i) => i.id === invoiceId);
  if (!invoice) throw new Error(`Invoice ${invoiceId} not found`);
  invoice.payments = invoice.payments || [];
  const payment = { id: db.nextId('pay'), amount, method, date: new Date().toISOString(), reference: extra.reference || '' };
  ...
  workflow.recordWorkflowEvent('invoice', invoice.id, method === 'credit_note' ? 'refund_issued' : 'payment_recorded',
    `${method === 'credit_note' ? 'Credit applied' : 'Payment'} of ${util.fmtMoney(amount)} on ${invoice.number}`,
    { customerId: invoice.customerId, invoiceId: invoice.id, paymentId: payment.id });
  return invoice;
};

// ---------------------------------------------------------------------------
// InvoiceOps finance dashboard — receivables aging, sales/payment summary,
// daily closeout, expenses, credit notes/refunds, items/services catalog,
// and customer balance summaries. All computed from db.invoices()/
// db.expenses()/db.creditNotes() — nothing here mutates an invoice except
// util.applyCreditNoteToInvoice (explicitly, via the existing payments-array
// pattern) and the credit-note/expense CRUD functions below.
// ---------------------------------------------------------------------------
const DEFAULT_TERMS_DAYS = 14;

function invoiceDueDate(inv) {
  if (inv.dueAt) return new Date(inv.dueAt);
  const issued = new Date(inv.issuedAt);
  issued.setDate(issued.getDate() + DEFAULT_TERMS_DAYS);
  return issued;
}

// Real, from live invoice balances/due dates. An invoice with no balance
// (paid/void) never counts toward receivables.
util.receivablesAging = () => {
  const now = Date.now();
  const open = db.invoices().filter((i) => i.balance > 0);
  const buckets = { current: { total: 0, count: 0 }, d1_15: { total: 0, count: 0 }, d16_30: { total: 0, count: 0 }, d31_45: { total: 0, count: 0 }, d45plus: { total: 0, count: 0 } };
  open.forEach((inv) => {
    const daysPastDue = Math.floor((now - invoiceDueDate(inv).getTime()) / 86400000);
    const bucket = daysPastDue <= 0 ? 'current' : daysPastDue <= 15 ? 'd1_15' : daysPastDue <= 30 ? 'd16_30' : daysPastDue <= 45 ? 'd31_45' : 'd45plus';
    buckets[bucket].total = round2(buckets[bucket].total + inv.balance);
    buckets[bucket].count += 1;
  });
  const totalReceivables = round2(open.reduce((s, i) => s + i.balance, 0));
  const overdue = round2(totalReceivables - buckets.current.total);
  return { totalReceivables, current: buckets.current.total, overdue, buckets, invoiceCount: open.length };
};
...
function periodRange(period) {
  ...
  else if (period === 'week') { start.setDate(start.getDate() - start.getDay() + 1); start.setHours(0, 0, 0, 0); }
  else if (period === 'month') { start.setDate(1); start.setHours(0, 0, 0, 0); }
  else if (period === 'year') { start.setMonth(start.getMonth() - 12); }
  else return { start: null, end: null }; // 'all'
  return { start, end: now };
}

// Real for everything except avgDaysToPay, which is explicitly badged a
// placeholder in the UI (computing it correctly needs a payment-date-minus-
// issued-date average across paid invoices in range — left simple/real
// here, but the UI still treats this section as "estimate" framing like the
// rest of the app's labor-cost numbers).
util.invoiceSalesSummary = (period = 'month') => {
  const { start, end } = periodRange(period);
  const inRange = (iso) => !start || (new Date(iso) >= start && new Date(iso) <= end);
  const invoices = db.invoices().filter((i) => inRange(i.issuedAt));
  const totalInvoiced = round2(invoices.reduce((s, i) => s + i.total, 0));
  const totalPaid = round2(invoices.reduce((s, i) => s + i.amountPaid, 0));
  const totalOutstanding = round2(invoices.reduce((s, i) => s + i.balance, 0));
  const refunds = round2(db.creditNotes().filter((c) => c.reason === 'refund' && c.status !== 'void' && inRange(c.createdAt)).reduce((s, c) => s + c.amount, 0));
  const deposits = round2(db.invoices().flatMap((i) => i.payments || []).filter((p) => p.method === 'deposit' && inRange(p.date)).reduce((s, p) => s + p.amount, 0));
  const paidDaysSamples = invoices.filter((i) => i.paidAt).map((i) => Math.max(0, Math.round((new Date(i.paidAt) - new Date(i.issuedAt)) / 86400000)));
  const avgDaysToPay = paidDaysSamples.length ? Math.round(paidDaysSamples.reduce((s, d) => s + d, 0) / paidDaysSamples.length) : null;
  return {
    totalInvoiced, totalPaid, totalOutstanding, refunds, deposits,
    unpaidBalance: totalOutstanding,
    avgInvoiceValue: invoices.length ? round2(totalInvoiced / invoices.length) : 0,
    avgDaysToPay, invoiceCount: invoices.length,
  };
};

// Flattens every invoice's payments array into one real, sortable list —
// no separate "payments" entity exists; this IS the Payments Received view.
util.allPaymentsReceived = () => {
  return db.invoices().flatMap((inv) => (inv.payments || []).map((p) => ({
    ...p, invoiceId: inv.id, invoiceNumber: inv.number, customerId: inv.customerId,
  })));
};

util.dailyCloseout = (dateIso) => {
  const date = (dateIso || new Date().toISOString()).slice(0, 10);
  const payments = util.allPaymentsReceived().filter((p) => p.date.slice(0, 10) === date);
  const byMethod = (m) => round2(payments.filter((p) => p.method === m).reduce((s, p) => s + p.amount, 0));
  const refunds = round2(db.creditNotes().filter((c) => c.status === 'issued' && c.createdAt.slice(0, 10) === date).reduce((s, c) => s + c.amount, 0));
  const invoicesPaidToday = db.invoices().filter((i) => i.paidAt && i.paidAt.slice(0, 10) === date).length;
  const cash = byMethod('cash'), card = byMethod('card'), check = byMethod('check'), deposits = byMethod('deposit'), ach = byMethod('ach');
  const netCollected = round2(cash + card + check + deposits + ach - refunds);
  return { date, cash, card, ach, check, financing: 0, deposits, refunds, netCollected, invoicesPaidToday, paymentExceptions: [] };
};

// ---- Expenses (tracking foundation, not real accounting) ----
util.createExpense = ({ date, vendor, category, amount, paymentMethod, linkedPoId, linkedInventoryItemId, notes }) => {
  const expenses = db.expenses();
  const expense = {
    id: db.nextId('exp'), date: date || new Date().toISOString().slice(0, 10), vendor: vendor || '', category, amount: Number(amount) || 0,
    paymentMethod: paymentMethod || 'card', linkedPoId: linkedPoId || null, linkedInventoryItemId: linkedInventoryItemId || null,
    notes: notes || '', status: 'recorded',
  };
  expenses.push(expense);
  db.saveExpenses(expenses);
  return expense;
};
util.setExpenseStatus = (expenseId, status) => {
  const expenses = db.expenses();
...
util.createCreditNote = ({ customerId, invoiceId, amount, reason, notes }) => {
  const notesList = db.creditNotes();
  const creditNote = {
    id: db.nextId('cn'), customerId, invoiceId: invoiceId || null, amount: Number(amount) || 0,
    reason: reason || 'adjustment', status: 'draft', createdAt: new Date().toISOString(), appliedAt: null, notes: notes || '',
  };
  notesList.push(creditNote);
  db.saveCreditNotes(notesList);
  workflow.recordWorkflowEvent('credit_note', creditNote.id, 'credit_note_created', `Credit note drafted (${util.fmtMoney(creditNote.amount)})`, { customerId, invoiceId: creditNote.invoiceId });
  return creditNote;
};
util.issueCreditNote = (creditNoteId) => {
  const notesList = db.creditNotes();
  const cn = notesList.find((c) => c.id === creditNoteId);
  if (!cn) throw new Error(`Credit note ${creditNoteId} not found`);
  if (cn.status !== 'draft') throw new Error('Only a draft credit note can be issued.');
  cn.status = 'issued';
  db.saveCreditNotes(notesList);
  workflow.recordWorkflowEvent('credit_note', cn.id, 'refund_issued', `Credit note issued (${util.fmtMoney(cn.amount)})`, { customerId: cn.customerId, invoiceId: cn.invoiceId });
  return cn;
};
// Applies an issued credit note as a payment-like reduction against an
// invoice's balance — uses the SAME payments-array mechanism as
// util.recordPayment (method: 'credit_note') so the invoice's
// amountPaid/balance/status stay internally consistent. Never deletes or
// rewrites the original invoice.
util.applyCreditNoteToInvoice = (creditNoteId, invoiceId) => {
  const notesList = db.creditNotes();
  const cn = notesList.find((c) => c.id === creditNoteId);
  if (!cn) throw new Error(`Credit note ${creditNoteId} not found`);
  if (cn.status !== 'issued') throw new Error('Only an issued credit note can be applied.');
  const invoice = db.invoiceById(invoiceId);
  if (!invoice) throw new Error(`Invoice ${invoiceId} not found`);
  util.recordPayment(invoiceId, Math.min(cn.amount, invoice.balance), 'credit_note', { reference: cn.id });
  cn.status = 'applied';
  cn.invoiceId = invoiceId;
  cn.appliedAt = new Date().toISOString();
  db.saveCreditNotes(notesList);
  return cn;
};
util.voidCreditNote = (creditNoteId) => {
  const notesList = db.creditNotes();
  const cn = notesList.find((c) => c.id === creditNoteId);
  if (!cn) throw new Error(`Credit note ${creditNoteId} not found`);
  if (cn.status === 'applied') throw new Error('An applied credit note cannot be voided — it has already adjusted an invoice balance.');
  cn.status = 'void';
  db.saveCreditNotes(notesList);
  return cn;
};

// ---- Items / Services catalog (reusable invoice/estimate line items —
// separate from db.services(), which is the RO-specific service list) ----
util.createInvoiceItem = ({ name, type, sku, defaultPrice, defaultCost, taxable, linkedInventoryItemId }) => {
  const items = db.invoiceItems();
  const item = {
    id: db.nextId('fi'), name, type, sku: sku || '', defaultPrice: Number(defaultPrice) || 0, defaultCost: Number(defaultCost) || 0,
    taxable: !!taxable, active: true, linkedInventoryItemId: linkedInventoryItemId || null,
  };
  items.push(item);
  db.saveInvoiceItems(items);
  return item;
};
util.updateInvoiceItem = (itemId, patch) => {
  const items = db.invoiceItems();
...
util.setInvoiceItemActive = (itemId, active) => util.updateInvoiceItem(itemId, { active });

// ---- Customer finance summary ----
util.customerFinanceSummary = (customerId) => {
  const invoices = db.invoices().filter((i) => i.customerId === customerId);
  const unpaidInvoices = invoices.filter((i) => i.balance > 0);
  const payments = invoices.flatMap((i) => (i.payments || []).map((p) => ({ ...p, invoiceNumber: i.number })));
  const credits = db.creditNotesForCustomer(customerId);
  const deposits = payments.filter((p) => p.method === 'deposit');
  const totalBalance = round2(unpaidInvoices.reduce((s, i) => s + i.balance, 0));
  const lastPayment = payments.slice().sort((a, b) => new Date(b.date) - new Date(a.date))[0];
  return {
    unpaidInvoices, payments, credits, deposits, totalBalance,
...
util.cancelRO = (roId) => {
  ...
  ro.status = 'cancelled';
  const saved = saveJob(ro);
  workflow.recordWorkflowEvent('job', ro.id, 'ro_cancelled', `${ro.ro} cancelled`, { customerId: ro.customerId, roId: ro.id });
  return saved;
};
...
util.createQuote = (data) => {
  ...
  const q = util.recalcQuote({
    ...
    roId: null,
    // §8 — optional CRM lead link (no standalone Opportunity entity exists
    // in this codebase; a Lead is the closest real record). Additive field,
    // null for the existing manual/booking/dvi quote-creation paths.
  ...
  quotes.push(q);
  db.saveQuotes(quotes);
  workflow.recordWorkflowEvent('quote', q.id, 'quote_created', `${q.quoteNumber} created`, { customerId: q.customerId, vehicleId: q.vehicleId, quoteId: q.id, leadId: q.leadId });
  if (q.leadId) workflow.linkEntities('lead', q.leadId, 'quote', q.id, 'crm_lead_to_quote');
  return q;
};

// §7 (cross-module workflow) — turn selected red/yellow DVI findings
// (RO.recommended — this codebase has no standalone DVI entity; the RO's
// recommended-items list IS the DVI findings list) into a real Quote.
// Idempotent: a recommended item already converted carries
// `convertedToQuoteId` and is skipped on a second call, so re-running this
// (e.g. a duplicate click) never creates duplicate quote lines.
util.createQuoteFromRecommended = (roId, itemNames) => {
  const ro = db.jobById(roId);
  if (!ro) throw new Error(`RO ${roId} not found`);
  const candidates = (ro.recommended || []).filter((r) => !r.convertedToQuoteId && (!itemNames || itemNames.includes(r.name)));
  if (!candidates.length) throw new Error('No un-converted recommended items to build a quote from.');

  const quote = util.createQuote({
    customerId: ro.customerId,
    vehicleId: ro.vehicleId,
    title: 'Recommended work',
    concern: `From ${ro.ro} inspection findings`,
    advisorId: ro.advisorId,
    techId: ro.techId,
    source: 'dvi',
    lineItems: candidates.map((r) => ({ type: 'service', name: r.name, qty: 1, unitPrice: r.price || 0, hours: r.hours || 0, source: 'dvi_finding', sourceRoId: ro.id })),
  });

  const jobs = db.jobs();
  const job = jobs.find((j) => j.id === ro.id);
  (job.recommended || []).forEach((r) => { if (candidates.includes(r) || candidates.some((c) => c.name === r.name)) r.convertedToQuoteId = quote.id; });
  db.saveJobs(jobs);

  // util.createQuote() above already fired a generic 'quote_created' event;
  // just add the DVI link here so the chain (RO finding -> quote) is visible
  // without logging a second, redundant "created" timeline entry.
  workflow.linkEntities('job', ro.id, 'quote', quote.id, 'dvi_to_quote');
  return quote;
};
...
// `status`: 'approved' | 'declined' | 'deferred' (or 'recommended'/'optional'
// to revert). Shop-side quick action — does NOT recompute the quote's own
// top-level status (see util.submitQuoteApproval for the customer-facing
// flow that does), it only marks one line and logs the activity.
const LINE_EVENT_TYPE = { approved: 'quote_item_approved', declined: 'quote_item_declined', deferred: 'quote_item_deferred' };
util.setQuoteLineItemStatus = (quoteId, lineId, status) => {
  const q = db.quoteById(quoteId);
  const line = (q.lineItems || []).find((l) => l.id === lineId);
  if (line) line.status = status;
  util.recalcQuote(q);
  const saved = saveQuote(q);
  if (line && LINE_EVENT_TYPE[status]) {
    workflow.recordWorkflowEvent('quote', q.id, LINE_EVENT_TYPE[status], `${line.name} marked ${status} on ${q.quoteNumber}`, { customerId: q.customerId, quoteId: q.id });
  }
  return saved;
};
...
util.sendQuote = (quoteId) => {
  ...
  const saved = saveQuote(q);
  workflow.recordWorkflowEvent('quote', q.id, 'quote_sent', `${q.quoteNumber} sent`, { customerId: q.customerId, quoteId: q.id });
  // §8 — a quote sent but not yet approved needs a follow-up; dedupeKey
  // keeps re-sending the same quote from stacking duplicate follow-ups.
  workflow.createFollowUpTask({
    title: `Follow up on ${q.quoteNumber}`, reason: 'Quote sent, awaiting customer response',
    customerId: q.customerId, relatedType: 'quote', relatedId: q.id,
    dueAt: new Date(Date.now() + 3 * 86400000).toISOString(), ownerId: q.advisorId,
    dedupeKey: `quote_followup_${q.id}`,
  });
  return saved;
};

// §2/§6 — "Log Send" from the Send Quote modal's email/text preview. If the
// quote hasn't been sent yet this also runs the real sendQuote transition
// (creating the follow-up task); either way it logs a channel-specific
// delivery event so email vs. text vs. print are distinguishable in the
// CRM timeline. `channel`: 'email' | 'sms'.
util.logQuoteDelivery = (quoteId, channel) => {
  const q = db.quoteById(quoteId);
  if (['draft', 'review_required', 'ready_to_send'].includes(q.status)) util.sendQuote(quoteId);
  const eventType = channel === 'sms' ? 'quote_sent_sms' : 'quote_sent_email';
  const label = channel === 'sms' ? 'text preview' : 'email preview';
  workflow.recordWorkflowEvent('quote', quoteId, eventType, `${q.quoteNumber} sent by ${label}`, { customerId: q.customerId, quoteId, channel: channel === 'sms' ? 'sms_preview' : 'email_preview' });
  return db.quoteById(quoteId);
};
...
util.markQuoteViewed = (quoteId) => {
  ...
  q.updatedAt = new Date().toISOString();
  const saved = saveQuote(q);
  workflow.recordWorkflowEvent('quote', q.id, 'quote_viewed', `${q.quoteNumber} viewed by customer`, { customerId: q.customerId, quoteId: q.id });
  return saved;
};
...
util.approveQuote = (quoteId) => {
  ...
  util.recalcQuote(q);
  const saved = saveQuote(q);
  workflow.recordWorkflowEvent('quote', q.id, 'quote_approved', `${q.quoteNumber} approved`, { customerId: q.customerId, quoteId: q.id });
  workflow.completeFollowUpTasksFor('quote', q.id);
  return saved;
};
...
util.partiallyApproveQuote = (quoteId, approvedLineIds) => {
  ...
  util.recalcQuote(q);
  const saved = saveQuote(q);
  workflow.recordWorkflowEvent('quote', q.id, 'quote_partially_approved', `${q.quoteNumber} partially approved`, { customerId: q.customerId, quoteId: q.id });
  workflow.completeFollowUpTasksFor('quote', q.id);
  workflow.createFollowUpTask({
    title: `Declined-work follow-up — ${q.quoteNumber}`, reason: 'Some line items declined on partial approval',
...
util.declineQuote = (quoteId, reason) => {
  ...
  util.recalcQuote(q);
  const saved = saveQuote(q);
  workflow.recordWorkflowEvent('quote', q.id, 'quote_declined', `${q.quoteNumber} declined`, { customerId: q.customerId, quoteId: q.id, description: reason || '' });
  workflow.completeFollowUpTasksFor('quote', q.id);
  workflow.createFollowUpTask({
    title: `Declined-work follow-up — ${q.quoteNumber}`, reason: reason || 'Quote declined',
    customerId: q.customerId, relatedType: 'quote', relatedId: q.id, ownerId: q.advisorId,
    dueAt: new Date(Date.now() + 14 * 86400000).toISOString(),
    dedupeKey: `quote_declined_followup_${q.id}`,
  });
...
// §4/§5 — the customer-approval-preview's per-line decision flow. Unlike
// approveQuote/partiallyApproveQuote/declineQuote (still here, unchanged,
// used by the shop's quick "Approve All"/"Decline" buttons), this supports a
// THIRD line outcome — 'deferred' ("decide later") — with no backing
// top-level quote status of its own. `decisions`: { [lineItemId]:
// 'approved'|'declined'|'deferred' }, one entry per line the customer acted
// on (lines not present keep their current status). `note`: optional
// customer question/comment, stored on q.customerNotes and logged as its
// own activity event.
util.submitQuoteApproval = (quoteId, decisions, note) => {
  const q = db.quoteById(quoteId);
  requireQuoteStatus(q, ['sent', 'viewed']);
  const originalStatus = q.status;
  const before = new Map(q.lineItems.map((l) => [l.id, l.status]));
  q.lineItems.forEach((l) => {
    const d = decisions[l.id];
    if (d === 'approved' || d === 'declined' || d === 'deferred') l.status = d;
  });

  const total = q.lineItems.length;
  const approvedCount = q.lineItems.filter((l) => l.status === 'approved').length;
  const declinedCount = q.lineItems.filter((l) => l.status === 'declined').length;

  let newStatus = originalStatus; // default: nothing finalized (e.g. all deferred) — leave as sent/viewed
  if (approvedCount === total) newStatus = 'approved';
  else if (declinedCount === total) newStatus = 'declined';
  else if (approvedCount > 0) newStatus = 'partially_approved';
  q.status = newStatus;
  if (newStatus === 'approved' || newStatus === 'partially_approved') q.approvedAt = new Date().toISOString();
  if (newStatus === 'declined') q.declinedAt = new Date().toISOString();
  if (note) q.customerNotes = note;
  util.recalcQuote(q);
  const saved = saveQuote(q);

  q.lineItems.forEach((l) => {
    if (before.get(l.id) !== l.status && LINE_EVENT_TYPE[l.status]) {
      workflow.recordWorkflowEvent('quote', q.id, LINE_EVENT_TYPE[l.status], `Customer marked "${l.name}" ${l.status} on ${q.quoteNumber}`, { customerId: q.customerId, quoteId: q.id, actorType: 'customer' });
    }
  });
  if (note) {
    workflow.recordWorkflowEvent('quote', q.id, 'quote_note_submitted', `Customer note on ${q.quoteNumber}`, { customerId: q.customerId, quoteId: q.id, actorType: 'customer', description: note });
  }
  if (newStatus !== originalStatus) {
    if (newStatus === 'approved') workflow.recordWorkflowEvent('quote', q.id, 'quote_approved', `${q.quoteNumber} approved by customer`, { customerId: q.customerId, quoteId: q.id, actorType: 'customer' });
    else if (newStatus === 'partially_approved') workflow.recordWorkflowEvent('quote', q.id, 'quote_partially_approved', `${q.quoteNumber} partially approved by customer`, { customerId: q.customerId, quoteId: q.id, actorType: 'customer' });
    else if (newStatus === 'declined') workflow.recordWorkflowEvent('quote', q.id, 'quote_declined', `${q.quoteNumber} declined by customer`, { customerId: q.customerId, quoteId: q.id, actorType: 'customer' });
  }
  if (['approved', 'partially_approved'].includes(newStatus)) workflow.completeFollowUpTasksFor('quote', q.id);
  if (newStatus === 'declined' || (newStatus === 'partially_approved' && declinedCount > 0)) {
    workflow.createFollowUpTask({
      title: `Declined-work follow-up — ${q.quoteNumber}`, reason: 'Customer declined or deferred items on approval preview',
      customerId: q.customerId, relatedType: 'quote', relatedId: q.id, ownerId: q.advisorId,
      dueAt: new Date(Date.now() + 14 * 86400000).toISOString(),
      dedupeKey: `quote_declined_followup_${q.id}`,
    });
...
util.convertQuoteToRO = (quoteId, opts = {}) => {
  ...
  workflow.linkEntities('quote', q.id, 'job', ro.id, 'quote_to_ro');
  workflow.recordWorkflowEvent('job', ro.id, 'ro_created', `${ro.ro} created from ${q.quoteNumber}`, { customerId: q.customerId, roId: ro.id, quoteId: q.id });

  return ro;
};

// §1/§9 — "Convert to Invoice" from the quote detail view. There is no
// direct quote->invoice path in this codebase by design: invoicing requires
// the work to actually be DONE (RO status 'ready'), which only happens after
// convertQuoteToRO and the real shop-floor lifecycle (checkIn/startJob/
// markReady). So this is "safe" only once that's true — otherwise it throws
// a clear, specific error rather than skipping the fulfillment step.
util.createInvoiceFromQuote = (quoteId) => {
  const q = db.quoteById(quoteId);
  if (!q) throw new Error(`Quote ${quoteId} not found`);
  // q.roId only ever gets set by convertQuoteToRO, which atomically sets
  // q.status to 'converted' at the same time — so by the time roId exists,
  // the quote's own status is already past approved/partially_approved.
  // The real remaining gate is whether that RO has actually been finished.
  if (!q.roId) throw new Error(`${q.quoteNumber} hasn't been converted to a repair order yet — convert it first, complete the work, then invoice.`);
  const ro = db.jobById(q.roId);
  if (!ro || ro.status !== 'ready') throw new Error(`${ro?.ro || q.roId} isn't marked ready yet — the work has to be completed before invoicing.`);
  return util.createInvoiceFromRO(q.roId); // already links quote_to_invoice/ro_to_invoice + logs the event
};

// §9 — when a quote was built FROM an existing RO's DVI findings
// (createQuoteFromRecommended) and the customer approves it, this adds the
// approved lines onto that SAME RO instead of spinning up a new one.
// Idempotent: each RO line item tagged with sourceQuoteLineId, so calling
// this twice for the same quote never double-adds a line.
util.addApprovedQuoteItemsToRO = (quoteId, roId) => {
  const q = db.quoteById(quoteId);
  if (!q) throw new Error(`Quote ${quoteId} not found`);
  const ro = db.jobById(roId);
  if (!ro) throw new Error(`RO ${roId} not found`);
  if (util.isROLocked(ro)) throw new Error(`${ro.ro} is ${ro.status} — cannot add approved work.`);

  const alreadyAdded = new Set((ro.lineItems || []).map((l) => l.sourceQuoteLineId).filter(Boolean));
  const approvedLines = q.lineItems.filter((l) => l.status === 'approved' && !alreadyAdded.has(l.id));
  if (!approvedLines.length) throw new Error('No newly-approved items to add — already on this RO, or nothing approved.');

  approvedLines.forEach((l) => {
    util.addLineItem(ro.id, { type: l.type === 'labor' ? 'labor' : (l.partId ? 'part' : 'service'), refId: l.partId || l.refId || null, name: l.name, qty: l.qty || 1, unitPrice: l.unitPrice || 0, hours: l.hours || 0, sourceQuoteId: q.id, sourceQuoteLineId: l.id });
  });

  workflow.linkEntities('quote', q.id, 'job', ro.id, 'quote_to_ro');
  workflow.recordWorkflowEvent('job', ro.id, 'ro_approved_work_added', `${approvedLines.length} approved item(s) from ${q.quoteNumber} added to ${ro.ro}`, { customerId: ro.customerId, roId: ro.id, quoteId: q.id });
  return db.jobById(roId);
};
...
const MODULE_LIST = [
  'Dashboard', 'Booking', 'Appointments', 'CRM', 'Customer Care', 'Quotes', 'Digital Inspections', 'Repair Orders',
...
// Quotes corrections (quote send/approval/CRM-chain task): service_manager
// none->limited (can view/approve/convert per role spec, not delete/admin),
// technician/apprentice none->read_only (view assigned quote/inspection
// recommendations, no pricing/send), marketing none->read_only (view quote
// activity for declined-work follow-up, no pricing edits). Every other role
// below was already correct against that same spec and is unchanged.
const MODULE_ACCESS_BY_ROLE = {
  ...
  service_manager: { Dashboard: 'limited', Booking: 'none', Appointments: 'read_only', CRM: 'limited', Quotes: 'limited', 'Digital Inspections': 'full', 'Repair Orders': 'full', 'Live Monitor': 'full', Invoices: 'none', POS: 'none', Inventory: 'limited', Marketing: 'none', Team: 'limited', Schedule: 'full', 'PTO / Time Off': 'limited', 'Time Clock': 'full', Reports: 'limited', Settings: 'none', 'Billing/Subscription': 'none', Integrations: 'none', 'Data Export': 'none' },
  ...
  technician: { Dashboard: 'limited', Booking: 'none', Appointments: 'read_only', CRM: 'none', Quotes: 'read_only', 'Digital Inspections': 'full', 'Repair Orders': 'limited', 'Live Monitor': 'read_only', Invoices: 'none', POS: 'none', Inventory: 'read_only', Marketing: 'none', Team: 'none', Schedule: 'limited', 'PTO / Time Off': 'limited', 'Time Clock': 'full', Reports: 'none', Settings: 'none', 'Billing/Subscription': 'none', Integrations: 'none', 'Data Export': 'none' },
  apprentice: { Dashboard: 'limited', Booking: 'none', Appointments: 'read_only', CRM: 'none', Quotes: 'read_only', 'Digital Inspections': 'limited', 'Repair Orders': 'read_only', 'Live Monitor': 'read_only', Invoices: 'none', POS: 'none', Inventory: 'read_only', Marketing: 'none', Team: 'none', Schedule: 'limited', 'PTO / Time Off': 'limited', 'Time Clock': 'full', Reports: 'none', Settings: 'none', 'Billing/Subscription': 'none', Integrations: 'none', 'Data Export': 'none' },
  ...
  marketing: { Dashboard: 'none', Booking: 'none', Appointments: 'none', CRM: 'limited', Quotes: 'read_only', 'Digital Inspections': 'none', 'Repair Orders': 'none', 'Live Monitor': 'none', Invoices: 'none', POS: 'none', Inventory: 'none', Marketing: 'full', Team: 'none', Schedule: 'none', 'PTO / Time Off': 'none', 'Time Clock': 'none', Reports: 'limited', Settings: 'none', 'Billing/Subscription': 'none', Integrations: 'none', 'Data Export': 'none' },
...
// Convenience wrapper used by page modules to gate buttons — same demo-role
// pattern lib/nav.js already uses for "View app as" (the currently "logged
// in" employee is db.settings().currentUserId). `moduleName` must be one of
// MODULE_LIST (e.g. 'Quotes'); `action` one of view/create/edit/delete/
// approve/export/admin.
util.currentRole = () => db.employeeById(db.settings().currentUserId)?.role || null;
util.canUser = (moduleName, action) => util.actionsForAccessLevel(util.moduleAccessForRole(util.currentRole()).access[moduleName] || 'none').includes(action);
...
util.sendCampaign = (campaignId) => {
  ...
  // Cap linking to a sane number of recipients — a broad segment (e.g. "all
  // customers") doesn't need one entityLink per customer to prove the
  // campaign_to_customer relationship exists; communications themselves
  // (already in customerTimeline) carry the per-recipient record.
  recipients.slice(0, 50).forEach((customer) => workflow.linkEntities('campaign', campaign.id, 'customer', customer.id, 'campaign_to_customer'));
  workflow.recordWorkflowEvent('campaign', campaign.id, 'campaign_sent', `${campaign.name} sent to ${recipients.length} customer(s)`, {});
...
util.createPurchaseOrder = ({ supplierId, destinationLocationId, expectedDate, notes, items }) => {
  ...
  (items || []).forEach((it) => {
    ...
    workflow.linkEntities('purchase_order', po.id, 'inventory_item', it.partId, 'purchase_order_to_inventory');
  });
  db.savePurchaseOrderItems(poItems);
  workflow.recordWorkflowEvent('purchase_order', po.id, 'po_created', `${number} created`, { inventoryItemId: items?.[0]?.partId || null });
  return po;
};
...
util.receivePOItem = (poId, partId, qty) => {
  ...
  workflow.recordWorkflowEvent('purchase_order', poId, 'parts_received', `${receiveQty} received against ${po.number}`, { inventoryItemId: partId });
  return item;
};
...
util.markPOItemBackordered = (poId, partId, qty) => {
  ...
  workflow.recordWorkflowEvent('purchase_order', poId, 'parts_backordered', `${qty} backordered`, { inventoryItemId: partId });
  return item;
};
