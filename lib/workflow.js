...
export function reassignFollowUp(taskId, employeeId) {
  const tasks = db.followUpTasks();
  const task = tasks.find((t) => t.id === taskId);
  if (!task) throw new Error(`Follow-up ${taskId} not found`);
  const prevId = task.ownerId;
  task.ownerId = employeeId || null;
  db.saveFollowUpTasks(tasks);
  const emp = employeeId ? db.employeeById(employeeId) : null;
  recordWorkflowEvent('followup', taskId, 'followup_reassigned', emp ? `Follow-up reassigned to ${emp.firstName} ${emp.lastName}` : 'Follow-up unassigned', { customerId: task.customerId || null, metadata: { previousOwnerId: prevId, by: db.settings().currentUserId } });
  return task;
}
...
export function seedCrmDemoData() {
  ...
  // Demo: overdue follow-up for campaign lead (Kenji, lead_6)
  createFollowUpTask({
    title: 'Call Kenji back — tire rotation campaign lead',
    taskType: 'call', reason: 'Responded to campaign; needs booking follow-up',
    relatedType: 'lead', relatedId: 'lead_6', ownerId: 'e_sara',
    dueAt: new Date(Date.now() - 5 * 86400000).toISOString(), // 5 days overdue
    dedupeKey: 'demo_overdue_followup_lead6',
  });
  // Demo: follow-up due today for high-value fleet lead (lead_5)
  createFollowUpTask({
    title: 'Fleet Co. — send service contract quote',
    taskType: 'quote_follow_up', reason: 'High-value fleet opportunity; estimate requested',
    relatedType: 'lead', relatedId: 'lead_5', ownerId: 'e_omar',
    dueAt: new Date().toISOString(), // due today
    dedupeKey: 'demo_duetoday_followup_lead5',
  });
